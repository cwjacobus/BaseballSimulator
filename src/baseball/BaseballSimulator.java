...
public class BaseballSimulator {
	...
	public static void main(String[] args) {
		...
		ArrayList<ArrayList<ArrayList<Player>>> lineupBatters = setOptimalLineup(teamNames, years);
		...
		for (int s = 0; s < seriesLength; s++) {
			...
			for (int t = 0; t < 2; t++) {
				...
				if (Arrays.asList(nationalLeagueTeams).contains(teamNames[1]) || years[1] < 1973) {
					boxScores[t].getBatters().set(8, new ArrayList<Player>());  // Clear out prior games pitcher spots
					boxScores[t].getBatters().get(8).add(new Player(startingPitcher.getFullName(), startingPitcher.getMlbPlayerId(), "P")); // Set pitcher as batting ninth, if no DH
				}
			}
		...
		if (seriesLength > 1) { // Series results and calculations
			...
			System.out.println(boxScores[homeWinner].getTeamName() + ": " + totalWins.get(boxScores[homeWinner].getTeamName()) + "(" + df.format((double)totalWins.get(boxScores[homeWinner].getTeamName())/seriesLength) + 
				") " + boxScores[homeWinner==1?0:1].getTeamName() + ": " + totalWins.get(boxScores[homeWinner==1?0:1].getTeamName()) + "(" + df.format((double)totalWins.get(boxScores[homeWinner==1?0:1].getTeamName())/seriesLength) + ")");
	...
	private static void clearPlayerGameData(BoxScore bs) {
		for (int i = 0 ; i <= NUM_OF_PLAYERS_IN_LINEUP - 1; i++) {
			ArrayList<Player> posPlayers = bs.getBatters().get(i);
			for (Player p : posPlayers) {
				p.setBattingStats(new BattingStats());
			}
		}
	}
	...
	private static void updateBasesSituationRunnersAdvance(int event, Player currentBatter) {
	  /*int basesSituation = (currentBasesSituation << event) + (int)Math.pow(2, (event - 1));
		...
		ArrayList<Integer> runnersScoring = new ArrayList<Integer>();
		for (int e = 0; e < event; e++) {
			for (int b = 3; b >= 0; b--) {
				...
				if (b == 3 && gameState.isBaseOccupied(3)) {     // If runner on 3rd run scores
					runnersScoring.add(gameState.getRunnersOnBase()[2]);
				}
			}
		}
		if (runnersScoring.size() > 0) {
			/*System.out.println(runsScored + " RUNS SCORED - " + boxScores[0].getTeamName() + ": " + boxScores[0].getScore(gameState.getInning())  + 
				" " + boxScores[1].getTeamName() + ": " + boxScores[1].getScore(gameState.getInning()));*/
			runsScore(runnersScoring);
		}
	}
	...
	private static void updateBasesSituationNoRunnersAdvance(Player currentBatter) { 
		...
		if (gameState.getCurrentBasesSituation() == GameState.BASES_LOADED || gameState.getCurrentBasesSituation() == GameState.MAN_ON_FIRST_AND_SECOND) { // 123 or 12
			...
			if (gameState.getCurrentBasesSituation() == GameState.BASES_LOADED) {
				runsScore(gameState.getRunnersOnBase()[2]);
			}
	...
	private static void runsScore(int runnerid) {
		ArrayList<Integer> runnerIdList = new ArrayList<Integer>();
		runnerIdList.add(runnerid);  // For a single runner (many cases)
		runsScore(runnerIdList);
	}
	
	private static void runsScore(BaseRunner baseRunner) {  // single baseRunner
		ArrayList<BaseRunner> baseRunnerList = new ArrayList<BaseRunner>();
		baseRunnerList.add(baseRunner);  // For a single runner (many cases)
		//runsScore(baseRunnerList);
	}

	private static void runsScore(ArrayList<Integer> runnerIdList/*ArrayList<BaseRunner> baseRunnerList*/) {
		BoxScore boxScore = boxScores[gameState.getTop()];
		for (Integer runnerId : runnerIdList) {
			//Player pitcherResponsible = boxScore.getPitchers().get(baseRunnerList.get(i).getId());
			//int bo = getBattingOrderForPlayer(runnerId);
			//Player runner = boxScore.getBatters().get(bo - 1).get(boxScore.getBatters().get(bo - 1).size() - 1);
			Player runner = getPlayerFromId(runnerId);
			runner.getBattingStats().incrementRuns();
			//pitcherResponsible.getPitchingStats().incrementRunsAllowed();
		}
		boxScore.setRunsScored(gameState.getInning(), runnerIdList.size()); // run scores
		System.out.println(runnerIdList.size() + " RUNS SCORE - " + boxScores[0].getTeamName() + ": " + boxScores[0].getScore(gameState.getInning())  + " " + 
			boxScores[1].getTeamName() + ": " + boxScores[1].getScore(gameState.getInning()));
	}
	...
	private static int stealBase(int baseToSteal) {
		...
		if ((baseToSteal < 4 && safeOutStealRando < Math.round(stealPctg*10)) || (baseToSteal < 4 && safeOutStealRando == 10)) { // safe/out - rarely stealing home is safe
			...
			if (baseToSteal == 4) {
				runsScore(gameState.getRunnersOnBase()[runnerStealingIndex]);
			}
		}
		else { // out
	...
	// Lineup is hmoe/vis->batting order (1-9)->List of PLayers
	private static ArrayList<ArrayList<ArrayList<Player>>> setOptimalLineup(String[] teams, int[] years) {
			ArrayList<ArrayList<ArrayList<Player>>> batters = new ArrayList<ArrayList<ArrayList<Player>>>();
		...
		for (int t = 0; t < 2; t++) {
			...
			ofCount = 0;
			batters.add(new ArrayList<ArrayList<Player>>());
			// Get random starter 1-5
			for (int i = 1 ; i <= NUM_OF_PLAYERS_IN_LINEUP - 1; i++) {  // 1 - 8
				batters.get(t).add(new ArrayList<Player>());
				if (i == 1) { 
					statType = "SB";
				}
				else if (i == 3) {
				...
				while (true) {
					...
					boolean positionNeeded = (!positionsUsed.contains(player.getPrimaryPosition()) && !player.getPrimaryPosition().equals("OF")) || (player.getPrimaryPosition().equals("OF") && ofCount < 3);
			...
			batters.get(t).add(new ArrayList<Player>()); // For DH/P in 9
			if (useDH) { // DH always bats ninth 
				...
				batters.get(t).get(8).add(new Player(player.getFullName(), player.getMlbPlayerId(), player.getPrimaryPosition()));
			}
			System.out.println();
		}
		return batters;
